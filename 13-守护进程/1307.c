...
/*
   守护进程可以重读其配置文件的一种方法：使用sigwait和多线程
*/
int main(int argc, char *argv[]) 
{
    ...
    // 恢复SIGHUP，阻塞其他（SIGHUP和SIGTERM的默认动作是终止线程）
    sa.sa_handler = SIG_DFL; // 默认动作
    sigemptyset(&sa.sa_mask);
    sa.sa_flags = 0;
    if (sigaction(SIGHUP, &sa, NULL) < 0)
        printf("%s can't restore SIGHUP default", cmd);
    sigfillset(&mask);
    if ((err = pthread_sigmask(SIG_BLOCK, &mask, NULL)) != 0)
        perror("SIG_BLOCK error");
    
    // 创建线程处理SIGHUP和SIGTERM
