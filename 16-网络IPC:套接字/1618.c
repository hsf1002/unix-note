...
/**
 * 使用dup2使得STDIN_FILENO的子进程副本对/dev/null开放，
 * 使得STDOUT_FILENO和STDERR_FILENO的子进程副本对套接字端点开放
 * 当执行uptime时，命令将结果写到它的标准输出，该标准输出连接到套接字
 * 所以数据被送到ruptime客户端命令
 */
void
serve(int sockfd)
{
    ...
    for (;;)
    {
        ...
        else if(pid == 0)
        {
            ...
            {
                perror("ruptimed: unexpected error");
                exit(1);
            }
            close(clfd);
            execl(UPTIME, "uptime", (char *)0);
            fprintf(stderr, "unexpected error return from execl %s", strerror(errno));
        }
        // 父进程
        else
        {
...
/**
 * 服务器程序，用来提供uptime命令的输出到16-16的客户端程序
 */
int 
